# -*- coding: utf-8 -*-
"""
Created on Sat Sep  3 19:04:01 2022
    ...
    for file in files:
        ...
        if filepath.endswith(".jpg"):
            ...
            if measurements.mean(denoise_ubyte) > 0:
                ...
                l = len(file)
                img_save_path = './flair_hist/' + sub_path + '/' + file [:l-8] + '_hist_' + file [l-8:]
                plt.imsave(img_save_path, denoise_ubyte)
...
for i in range(b):
    for number in range(155):
        ...
        if img_seg_pixels == 0:
            ...
            hausdorff1_array.append(hd_1)
            hausdorff2_array.append(hd_2)
            ...
            edges_seg = cv2.Canny(img_flair, 100,255)
            edges_gt = cv2.Canny(img_seg, 100,255)
            indices_seg = np.where(edges_seg != [0])
            indices_gt = np.where(edges_gt != [0])
            coordinates_seg = list(zip(indices_seg[1], indices_seg[0]))
            coordinates_gt = list(zip(indices_gt[1], indices_gt[0]))
            hd_1 = directed_hausdorff(coordinates_seg, coordinates_gt)
            hd_2 = directed_hausdorff(coordinates_gt, coordinates_seg)
            ...
            num10_array.append(num10)
            num11_array.append(num11)
            hausdorff1_array.append(hd_1[0])
            hausdorff2_array.append(hd_2[0])
...
df = pd.DataFrame({'patient_img':patient_array, "resolution":num1_array, 'ground_truth_pixels':num2_array, 'otsu_pixels':num3_array, 
                   ...
                   'specificity':num10_array, 'dice_sorensen_coefficient':num11_array, 'hausdorff_distance_1':hausdorff1_array, 
                   'hausdorff_distance_2':hausdorff2_array})
filepath = 'Otsu_v2.xlsx'
